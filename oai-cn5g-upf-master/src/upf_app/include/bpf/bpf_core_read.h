...
#define bpf_core_enum_value_exists(enum_type, enum_value)                      \
  __builtin_preserve_enum_value(                                               \
      ___bpf_typeof(enum_type), enum_value, BPF_ENUMVAL_EXISTS)
#endif
...
#define bpf_core_enum_value(enum_type, enum_value)                             \
  __builtin_preserve_enum_value(                                               \
      *(typeof(enum_type)*) enum_value, BPF_ENUMVAL_VALUE)
#else
#define bpf_core_enum_value(enum_type, enum_value)                             \
  __builtin_preserve_enum_value(                                               \
      ___bpf_typeof(enum_type), enum_value, BPF_ENUMVAL_VALUE)
#endif
...
#define ___core_readN(fn, fn_ptr, dst, src, ...)                               \
  ___read_ptrs(fn_ptr, src, ___nolast(__VA_ARGS__)) ___read(                   \
      fn, dst, ___type(src, ___nolast(__VA_ARGS__)), __t,                      \
      ___last(__VA_ARGS__));
#define ___core_read(fn, fn_ptr, dst, src, a, ...)                             \
  ___apply(___core_read, ___empty(__VA_ARGS__))(                               \
      fn, fn_ptr, dst, src, a, ##__VA_ARGS__)
...
#define BPF_CORE_READ_USER_INTO(dst, src, a, ...)                              \
  ({___core_read(                                                              \
      bpf_core_read_user, bpf_core_read_user, dst, (src), a, ##__VA_ARGS__)})

/* Non-CO-RE variant of BPF_CORE_READ_INTO() */
#define BPF_PROBE_READ_INTO(dst, src, a, ...)                                  \
  ({___core_read(                                                              \
      bpf_probe_read_kernel, bpf_probe_read_kernel, dst, (src), a,             \
      ##__VA_ARGS__)})
...
#define BPF_PROBE_READ_USER_INTO(dst, src, a, ...)                             \
  ({___core_read(                                                              \
      bpf_probe_read_user, bpf_probe_read_user, dst, (src), a,                 \
      ##__VA_ARGS__)})
...
#define BPF_CORE_READ_STR_INTO(dst, src, a, ...)                               \
  ({___core_read(                                                              \
      bpf_core_read_str, bpf_core_read, dst, (src), a, ##__VA_ARGS__)})
...
#define BPF_CORE_READ_USER_STR_INTO(dst, src, a, ...)                          \
  ({___core_read(                                                              \
      bpf_core_read_user_str, bpf_core_read_user, dst, (src), a,               \
      ##__VA_ARGS__)})

/* Non-CO-RE variant of BPF_CORE_READ_STR_INTO() */
#define BPF_PROBE_READ_STR_INTO(dst, src, a, ...)                              \
  ({___core_read(                                                              \
      bpf_probe_read_kernel_str, bpf_probe_read_kernel, dst, (src), a,         \
      ##__VA_ARGS__)})
...
#define BPF_PROBE_READ_USER_STR_INTO(dst, src, a, ...)                         \
  ({___core_read(                                                              \
      bpf_probe_read_user_str, bpf_probe_read_user, dst, (src), a,             \
      ##__VA_ARGS__)})
